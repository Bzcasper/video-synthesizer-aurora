...
const UpgradeCard: React.FC<UpgradeCardProps> = ({ currentTier }) => {
  ...
  const handleUpgrade = async () => {
    if (currentTier === "free") {
      ...
      window.open(
        "mailto:support@auroravideosynth.com?subject=Enterprise%20Upgrade%20Inquiry",
        "_blank",
      );
    }
  };
  ...
  const handleManageSubscription = () => {
    ...
    toast({
      ...
      description:
        "This would take you to the Stripe Customer Portal to manage your subscription.",
    });
  };

  return (
    <Card
      className={`glass-panel h-full border-white/10 ${
        currentTier === "pro"
          ...
          <div className="space-y-2">
            ...
            {showDetails && (
              <>
                <div className="flex items-center gap-2">
                  <CheckCircle
                    className={`h-4 w-4 ${currentTier !== "free" ? "text-green-400" : "text-gray-500"}`}
                  />
                  <span
                    className={`text-sm ${currentTier !== "free" ? "text-white" : "text-gray-500"}`}
                  >
                    {currentTier === "free"
                ...
                <div className="flex items-center gap-2">
                  <CheckCircle
                    className={`h-4 w-4 ${currentTier === "enterprise" ? "text-green-400" : "text-gray-500"}`}
                  />
                  <span
                    className={`text-sm ${currentTier === "enterprise" ? "text-white" : "text-gray-500"}`}
                  >
                    {currentTier === "enterprise"
                      ? "Custom branding"
