...
export const SceneEditor: React.FC<SceneEditorProps> = ({ scenes, setScenes }) => {
  const addScene = () => {
    ...
    if (scenes.length === 0) {
      toast({
        title: "Scene added!",
        description: "Customize each scene to create your perfect video sequence.",
      });
    }
  };
    ...
    <div className="space-y-6">
      ...
      {hasNoScenes ? (
        <div className="bg-white/5 border border-white/10 rounded-lg p-8 text-center">
          <p className="text-gray-400 mb-4">No scenes created yet. Add your first scene to start customizing your video.</p>
          <Button 
            onClick={addScene} 
            variant="outline"
