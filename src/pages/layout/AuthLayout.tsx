...
const AuthLayout = ({
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        } else if (!requireAuth && hasSession && (location.pathname === '/login' || location.pathname === '/signup' || location.pathname === '/reset-password')) {
    ...
    const {
      ...
      } else if (!requireAuth && session && (location.pathname === '/login' || location.pathname === '/signup' || location.pathname === '/reset-password')) {
        setIsTransitioning(true);
        setTimeout(() => navigate('/dashboard'), 300);
      }
    });
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-aurora-black flex items-center justify-center">
        <div className="flex flex-col items-center justify-center space-y-4">
          ...
          <p className="text-aurora-white/70 text-sm animate-pulse">Verifying session...</p>
        </div>
      </div>;
  }
  return <div className="min-h-screen bg-aurora-black flex items-center justify-center overflow-hidden rounded-none">
      ...
      <div className="fixed top-6 left-0 right-0 flex justify-center pointer-events-none z-30">
        <div className="flex items-center space-x-2">
          <div className="relative">
            <img src="/lovable-uploads/90dade48-0a3d-4761-bf1d-ff00f22a3a23.png" alt="Aurora Video Synth" className="h-8 w-8 object-contain" />
            <div className="absolute inset-0 bg-gradient-to-r from-aurora-purple via-aurora-blue to-aurora-green opacity-50 blur-lg -z-10" />
          </div>
