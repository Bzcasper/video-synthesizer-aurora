...
const validateRequest = (data: any): data is EnhancementRequest => {
  ...
  if (
    data.enhancement_type === "filter" &&
    data.filter_type &&
    !validFilterTypes.includes(data.filter_type)
  ) {
  ...
  if (
    ...
    !validResolutions.includes(data.target_resolution)
  ) {
    throw new Error("Invalid target_resolution");
  }

  if (
    data.enhancement_type === "speed_adjustment" &&
    data.speed_factor &&
    (data.speed_factor < 0.1 || data.speed_factor > 4.0)
  ) {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isPro) {
      ...
      if (count && count >= 5) {
        throw new Error(
          "Free tier limit reached. Please upgrade to Pro for unlimited enhancements.",
        );
      }

      // Restrict certain features for free tier
      if (
        ["upscale", "frame_interpolation"].includes(
          requestData.enhancement_type,
        )
      ) {
        throw new Error(
          "This enhancement type is only available for Pro users",
        );
      }
    }
