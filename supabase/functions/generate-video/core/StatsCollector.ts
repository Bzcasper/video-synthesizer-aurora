...
export class StatsCollector {
  ...
  async getUserMetrics(userId: string): Promise<UserJobMetrics> {
    try {
      ...
      if (error) {
        throw this.errorHandler.wrapError(error, `Failed to get jobs for user ${userId}`);
      }
  ...
  async getSystemMetrics(): Promise<SystemMetrics> {
    try {
      ...
      if (jobStatsError) {
        throw this.errorHandler.wrapError(jobStatsError, 'Failed to get job statistics');
      }
      ...
      const activeJobs = jobStats.filter(j => ['pending', 'processing'].includes(j.status)).length;
  ...
  async recordJobCompletion(
    ...
    try {
      ...
      logger.error(`Error recording job completion metrics for job ${jobId}:`, error);
