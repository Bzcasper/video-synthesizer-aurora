...
export class AssetManager {
  ...
  async saveFrames(jobId: string, frames: Uint8Array[]): Promise<string[]> {
    ...
    try {
      ...
      for (let i = 0; i < frames.length; i++) {
        ...
        if (error) {
          throw this.errorHandler.wrapError(
            error,
            `Failed to upload frame ${i} for job ${jobId}`,
          );
        }
        ...
        if (i % 10 === 0 || i === frames.length - 1) {
          logger.debug(
            `Saved ${i + 1}/${frames.length} frames for job ${jobId}`,
          );
        }
      }

      logger.info(
        `Successfully saved ${frames.length} frames for job ${jobId}`,
      );
      ...
      throw this.errorHandler.wrapError(
        error,
        `Failed to save frames for job ${jobId}`,
      );
    }
  }
  ...
  async saveVideoAssets(
    ...
    try {
      ...
      if (videoError) {
        throw this.errorHandler.wrapError(
          videoError,
          `Failed to upload video for job ${jobId}`,
        );
      }
      ...
      if (thumbnailError) {
        throw this.errorHandler.wrapError(
          thumbnailError,
          `Failed to upload thumbnail for job ${jobId}`,
        );
      }
      ...
      throw this.errorHandler.wrapError(
        error,
        `Failed to save video assets for job ${jobId}`,
      );
    }
  }
   ...
   * @private
   */
  private async recordAssets(
    jobId: string,
    urls: string[],
    type: "frame" | "video" | "thumbnail",
  ): Promise<void> {
    try {
      ...
      if (error) {
        logger.error(
          `Error recording assets in database for job ${jobId}:`,
          error,
        );
