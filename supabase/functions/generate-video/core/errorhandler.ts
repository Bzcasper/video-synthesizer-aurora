...
import { ERROR_TYPES, WEBHOOK_EVENTS, DB_TABLES, SYSTEM } from '../config/constants';
...
export class ErrorHandler {
  ...
  wrapError(
    ...
    if (error instanceof Error) {
      ...
      } else if (error.message.includes('system') || error.message.includes('internal')) {
        errorType = ERROR_TYPES.SYSTEM;
      }
    }
    
    // Determine if error is retryable based on type
    const isRetryable = options.isRetryable !== undefined ? options.isRetryable : 
      [ERROR_TYPES.TIMEOUT, ERROR_TYPES.SYSTEM, ERROR_TYPES.MODEL].includes(errorType);
  ...
  async handleVideoGenerationError(
    ...
    const wrappedError = error instanceof VideoGenerationError
      ? error
      : this.wrapError(error, 'Error during video generation');
    
    // Log the error
    logger.error(`Error processing job ${job.id} (${wrappedError.errorType}): ${wrappedError.message}`, {
      jobId: job.id,
    ...
    if (wrappedError.isRetryable) {
      ...
      if (retryCount < SYSTEM.MAX_RETRIES) {
        ...
        // Log retry attempt
        logger.info(`Retrying job ${job.id} (attempt ${retryCount + 1} of ${SYSTEM.MAX_RETRIES})`);
   ...
   * @private
   */
  private async updateJobFailure(job: VideoJob, error: VideoGenerationError): Promise<void> {
    try {
      ...
      if (updateError) {
        logger.error(`Error updating job failure for job ${job.id}:`, updateError);
      }
    } catch (dbError) {
   ...
   * @private
   */
  private async sendFailureWebhook(jobId: string, error: VideoGenerationError): Promise<void> {
    try {
      ...
      if (!webhookResponse.ok) {
        logger.warn(`Failure webhook for job ${jobId} returned status ${webhookResponse.status}: ${webhookResponse.statusText}`);
      } else {
        logger.debug(`Successfully sent failure webhook for job ${jobId}`);
      }
    } catch (webhookError) {
      logger.error(`Error sending failure webhook for job ${jobId}:`, webhookError);
    }
  }
  ...
  async logError(jobId: string, error: any, context?: any): Promise<void> {
    try {
      ...
      const errorDetails: ErrorDetails = {
        errorType: error instanceof VideoGenerationError ? error.errorType : ERROR_TYPES.UNKNOWN,
        ...
        isRetryable: error instanceof VideoGenerationError ? error.isRetryable : false,
        retryCount: this.retryMap.get(jobId) || 0,
        originalError: error instanceof VideoGenerationError ? error.originalError : error,
      };
      
      // Log error details
      logger.error(`Job ${jobId} error (${errorDetails.errorType}): ${errorDetails.message}`, errorDetails);
  ...
  isRetryableError(error: any): boolean {
    ...
    return retryablePatterns.some(pattern => errorString.toLowerCase().includes(pattern));
  }
