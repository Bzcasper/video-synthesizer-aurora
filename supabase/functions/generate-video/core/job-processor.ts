  ...
  async processJob(job: VideoJob): Promise<VideoGenerationResult> {
    ...
    try {
      ...
      if (job.settings.enhance_frames) {
        processedFrames = await this.frameEnhancer.enhanceFrames(frames, job.id);
      }
      ...
      const result = await this.videoAssembler.assembleVideo(processedFrames, job);
      ...
      throw this.errorHandler.wrapError(error, `Failed to process job ${job.id}`);
    }
  }
}
