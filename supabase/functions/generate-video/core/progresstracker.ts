...
export class ProgressTracker {
   ...
   */
  async updateProgress(jobId: string, progress: number, stage: string): Promise<void> {
   ...
   * @private
   */
  private async performProgressUpdate(jobId: string, progress: number, stage: string): Promise<void> {
    try {
      ...
      if (error) {
        throw this.errorHandler.wrapError(error, `Failed to update progress for job ${jobId}`);
      }
      
      // Log progress update
      logger.debug(`Updated progress for job ${jobId}: ${progress}% - ${stage}`);
   ...
   * @private
   */
  private async sendProgressWebhook(jobId: string, progress: number, stage: string): Promise<void> {
    try {
      ...
      if (!webhookResponse.ok) {
        logger.warn(`Webhook for job ${jobId} returned status ${webhookResponse.status}: ${webhookResponse.statusText}`);
      } else {
  ...
  async getJobProgress(jobId: string): Promise<{
    ...
    try {
      ...
      if (error) {
        throw this.errorHandler.wrapError(error, `Failed to get progress for job ${jobId}`);
      }
   ...
   */
  async recordEvent(jobId: string, event: string, details?: any): Promise<void> {
    try {
      // In a production system, you would store this in a job_events table
      // For now, we'll just log it
      logger.info(`[Job ${jobId}] ${event}${details ? ': ' + JSON.stringify(details) : ''}`);
    } catch (error) {
  ...
  async estimateTimeRemaining(
    ...
    try {
      ...
      // Log the estimate
      logger.debug(`Job ${jobId}: Estimated ${Math.round(remainingSeconds)}s remaining (${currentProgress}% complete in ${Math.round(elapsedSeconds)}s)`);
