...
export class QueueManager {
  ...
  async addJob(
    job: Omit<VideoJob, "id" | "createdAt" | "status" | "progress">,
  ): Promise<string> {
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      if (
        count &&
        count > 0 &&
        this.activeJobs.size < SYSTEM.MAX_CONCURRENT_JOBS
      ) {
  ...
  private async processJob(job: VideoJob): Promise<void> {
    ...
    try {
      ...
      await this.jobStatusManager.updateJobCompletion(
        job.id,
        result.videoUrl,
        result.thumbnailUrl,
      );
  ...
  private async handleJobTimeout(jobId: string): Promise<void> {
    logger.warn(
      `Job ${jobId} timed out after ${SYSTEM.JOB_TIMEOUT / 1000} seconds`,
    );
    ...
    try {
      ...
      if (error) {
        throw this.errorHandler.wrapError(
          error,
          `Failed to check status for job ${jobId}`,
        );
      }
