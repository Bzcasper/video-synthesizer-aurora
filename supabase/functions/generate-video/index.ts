    ...
    if (success) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message:
            "Job could not be cancelled. It may already be processing or completed.",
        }),
        { status: 409, headers },
      );
    }
    ...
    if (success) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          message:
            "Job could not be restarted. It may not be in a failed state.",
        }),
        { status: 409, headers },
      );
    }
 ...
 * @returns Response object
 */
async function handleQueueStatsRequest(
  headers: HeadersInit,
): Promise<Response> {
  try {
    // Get queue stats
