...
export class StorageUtils {
  ...
  async ensureBucketExists(
    bucketName: string = SYSTEM.STORAGE.BUCKET,
  ): Promise<boolean> {
    try {
      ...
      if (!buckets.find((b) => b.name === bucketName)) {
        ...
        if (createError) {
          throw this.errorHandler.wrapError(
            createError,
            `Failed to create storage bucket '${bucketName}'`,
          );
        }
      ...
      logger.error(
        `Error ensuring storage bucket '${bucketName}' exists:`,
        error,
      );
      return false;
    }
  ...
  getPublicUrl(
    ...
    try {
      ...
      if (options.transform) {
        urlData = this.supabase.storage.from(bucketName).getPublicUrl(path, {
